...
SELECT first_name, last_name, CONCAT("$",FORMAT(SUM(amount),2)) as 'Total Amount'
...
SELECT first_name, last_name, FORMAT(SUM(amount),2) as 'Total Amount Paid'
...
SELECT store.store_id, format(SUM(amount),2)  AS 'Money Made'
FROM store
INNER JOIN staff
...
SELECT name, FORMAT(SUM(amount),2) AS 'Gross Revenue'
FROM category 
